...
const composeTweet = tweet => dispatch => TweetAPIUtil.writeTweet(tweet).then(() => (dispatch(exports.receiveNewTweet(tweet)))).catch(err => console.log(err));
exports.composeTweet = composeTweet;
const fetchTweets = () => dispatch => {
    TweetAPIUtil.getTweets().then(tweets => (dispatch(exports.receiveTweets(tweets)))).catch(err => console.log(err));
};
exports.fetchTweets = fetchTweets;
const fetchTweet = id => dispatch => {
    debugger;
    TweetAPIUtil.getTweet(id).then(tweet => dispatch(exports.receiveTweet(tweet)).catch(err => console.log(err)));
};
exports.fetchTweet = fetchTweet;
const fetchUserTweets = id => dispatch => {
    TweetAPIUtil.getUserTweets(id).then(tweets => dispatch(exports.receiveUserTweets(tweets))).catch(err => console.log(err));
};
exports.fetchUserTweets = fetchUserTweets;
const likeTweet = (tweetId, userId) => dispatch => {
    return TweetAPIUtil.likeTweet(tweetId, userId)
        .then(tweet => dispatch(exports.receiveLike(tweet)));
};
exports.likeTweet = likeTweet;
const unlikeTweet = (tweetId, userId) => dispatch => {
